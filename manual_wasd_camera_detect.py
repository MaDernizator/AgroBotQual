...
# YOLO-детекция по нажатию M. Файл best.pt должен лежать рядом с этим скриптом.
DETECT_MODEL_PATH = Path(__file__).resolve().parent / "best.pt"
DETECT_CONF = 0.25
DETECT_IMGSZ = 640
...
# Состояние YOLO-детекции. M включает/выключает отрисовку результатов best.pt.
...
def load_yolo_model():
    """Ленивая загрузка ./best.pt: модель загружается только после первого нажатия M."""
    global yolo_model
    ...
    if not DETECT_MODEL_PATH.exists():
        raise FileNotFoundError(f"Не найден файл модели: {DETECT_MODEL_PATH}")

    from ultralytics import YOLO

    print(f"[YOLO] Загружаю модель: {DETECT_MODEL_PATH}")
    yolo_model = YOLO(str(DETECT_MODEL_PATH))
    return yolo_model


def toggle_detection() -> None:
    """M: включить/выключить отображение YOLO-детекции поверх кадра."""
    global detect_enabled

    if detect_enabled:
        detect_enabled = False
        print("[YOLO] Детекция выключена")
        return
    ...
    try:
        ...
        detect_enabled = False
        print(f"[YOLO] Не удалось включить детекцию: {exc}")
        print("[YOLO] Проверь, что best.pt лежит рядом со скриптом manual_wasd_camera.py")
        return

    detect_enabled = True
    print("[YOLO] Детекция включена")


def apply_detection_overlay(frame: np.ndarray) -> np.ndarray:
    """Возвращает кадр с bbox/классами от YOLO, если детекция включена."""
    global detect_enabled
    ...
    try:
        ...
        detect_enabled = False
        print(f"[YOLO] Ошибка детекции, выключаю M-режим: {exc}")
        return frame
...
def normalize_key(key):
    ...
    if ch in ("w", "a", "s", "d", "p", "q", "l", "k", "m"):
        return ch
    return None


def on_press(key):
    global record_toggle_requested, lift_toggle_requested, gripper_toggle_requested, detect_toggle_requested, exit_requested
    ...
    with state_lock:
        ...
        if k == "m" and not was_already_pressed:
            detect_toggle_requested = True
...
def pop_detect_toggle_request() -> bool:
    global detect_toggle_requested
    with state_lock:
        result = detect_toggle_requested
        detect_toggle_requested = False
    return result
...
def draw_overlay(frame: np.ndarray, is_recording: bool, movement: str, speed: int, video_path: Optional[Path]) -> np.ndarray:
    ...
    cv2.putText(display, "WASD move | Shift boost | P rec | L lift | K grab | M yolo | Q/Esc exit",
    ...
    yolo_text = "on" if detect_enabled else "off"
    cv2.putText(display, f"lift: {lift_text}   gripper: {gripper_text}   yolo: {yolo_text}",
...
def main():
    ...
    try:
        print("Управление: WASD — движение, Shift — скорость x2, P — старт/стоп записи, L — поднять/опустить захват, K — схватить/отпустить, M — YOLO-детекция через ./best.pt, Q или Esc — выход")
        ...
        while not should_exit():
            ...
            if frame is None:
                ...
                if writer is not None:
                    # В видео пишем кадр без интерфейсных надписей и без YOLO-боксов.
                    writer.write(frame)

            frame_for_display = apply_detection_overlay(frame_for_display)
